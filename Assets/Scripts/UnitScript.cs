...
public class UnitScript : NetworkBehaviour
{
    ...
    {
        // Get the GamePlayer of the requesting player
        ...
        //Check if the units selected are actually owned by the requesting player
        ...
        //Make sure there are not too many units selected and already occupying the land
        ...
        Debug.Log("CmdServerCanUnitsMove: player's requested move did not pass any checks. Defaulting to failure.");
        TargetReturnCanUnitsMove(connectionToClient, false, landUserClicked, positionToMoveTo);
        return;
    }
    [TargetRpc]
    public void TargetReturnCanUnitsMove(NetworkConnection target, bool serverCanMove, GameObject landUserClicked, Vector3 positionToMoveTo)
    {
        ...
        canMoveFromServer = serverCanMove;
        if (canMoveFromServer)
        {
            MoveAllUnits(landUserClicked, positionToMoveTo);
        }
        else if (!canMoveFromServer)
        {
    ...
    void MoveAllUnits(GameObject landUserClicked, Vector3 positionToMoveTo)
    {
        if (hasAuthority)
        {
            ...
            foreach (GameObject unit in MouseClickManager.instance.unitsSelected)
            {
                ...
                unit.GetComponent<UnitScript>().canMoveFromServer = false;
            }
    ...
    public void ResetUnitsMovement()
    {
        if (hasAuthority)
        {
            CmdUpdateUnitNewPosition(this.gameObject, this.startingPosition, this.previouslyOccupiedLand);
        }
    }
